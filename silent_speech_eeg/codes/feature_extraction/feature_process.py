...
def get_time_stamps(txt_paths = None, output_path= None):
    ...
    for path in tqdm(txt_paths):
        ...
        path = output_path + "/time_stamps/"
        if not os.path.exists(path):
            os.makedirs(path)
        time_stamps = pd.DataFrame({'start': start_list, 'stop': stop_list})
        time_stamps.to_csv(output_path + name + '.csv', index=False)
...
if __name__ == "__main__":
    ...
    if params.time_stamps:
        markers_paths = glob.glob(markers_path + "/*.txt")
        output_path = cwd + "/data/time_stamps/"
        if not os.path.exists(output_path):
            os.makedirs(output_path)
        get_time_stamps(txt_paths=markers_paths,
                        output_path= output_path)
    if params.trim_eeg:
        for ft in params.feature:
